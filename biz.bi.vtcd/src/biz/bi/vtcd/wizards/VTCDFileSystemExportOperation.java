 ...
 * @hcanedo
 */
public class VTCDFileSystemExportOperation implements IRunnableWithProgress {
	private IPath path;

    private IProgressMonitor monitor;

    private VTCDFileSystemExporter exporter = new VTCDFileSystemExporter();
     ...
     */
    public VTCDFileSystemExportOperation(List resources, String destinationPath,
     ...
     */
    public VTCDFileSystemExportOperation(IResource res, String destinationPath,
            IOverwriteQuery overwriteImplementor) {
        super();
        resource = res;
        path = new Path(destinationPath);
     ...
     *  directory creation)
     */
    public VTCDFileSystemExportOperation(IResource res, List resources,
    ...
    protected void exportFile(IFile file, IPath location)
        ...
        if (targetFile.exists()) {
            if (!targetFile.canWrite()) {
                errorTable.add(new Status(IStatus.ERROR, PlatformUI.PLUGIN_ID,
                		//TODO set message cannot overwrite
                        0, NLS.bind("cannotOverwrite", targetFile.getAbsolutePath()),
                        null));
                monitor.worked(1);
                return;
            }
        ...
        try {
            ...
            errorTable.add(new Status(IStatus.ERROR, PlatformUI.PLUGIN_ID, 0,
            		//TODO set message error Exporting
                    "", e));
        } catch (CoreException e) {
            errorTable.add(new Status(IStatus.ERROR, PlatformUI.PLUGIN_ID, 0,
//            		TODO set message error Exporting
    ...
    public IStatus getStatus() {
        ...
        return new MultiStatus(
                ...
                IStatus.OK,
                errors,//TODO set message error Exporting
                "",
                null);
    }
    ...
    public void run(IProgressMonitor progressMonitor)
        ...
        try {
            ...
            monitor.beginTask(//TODO set message  Exporting Title
                    "exportingTitle", totalWork);
            if (resourcesToExport == null) {
                exportAllResources();
            } else {
